...
- (void)setClockCenterImage:(CGImageRef)image
{
//	if (image == NULL) {
//		clockCenter.backgroundColor = [UIColor blackColor].CGColor;
//		clockCenter.cornerRadius = 3;
//	}else{
//		clockCenter.backgroundColor = [UIColor clearColor].CGColor;
//		clockCenter.cornerRadius = 0.0;
//		
//	}
	clockCenter.contents = (__bridge id)image;
}
...
- (void) layoutSubviews
{
    ...
    clockCenter.position = hourHand.position = minHand.position = secHand.position = c;
    ...
    if (clockCenter.contents == NULL) {
		w = SEC_HAND_WIDTH;
		h = length*SEC_HAND_LENGTH;
    }
    else
    {
        w = CGImageGetWidth((__bridge CGImageRef)clockCenter.contents)/scale;
		h = CGImageGetHeight((__bridge CGImageRef)clockCenter.contents)/scale;
    }
	clockCenter.bounds = CGRectMake(0,0,w,h);
    
	hourHand.anchorPoint = _hourPoint;
	minHand.anchorPoint = _minPoint;
	secHand.anchorPoint = _secPoint;
    clockCenter.anchorPoint = CGPointMake(0.5,0.5);
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
        ...
        [self setClockCenterImage:NULL];
		
		//add all created sublayers
        [containerLayer addSublayer:clockCenter];
