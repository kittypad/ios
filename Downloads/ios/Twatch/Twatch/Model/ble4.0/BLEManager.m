...
- (void)sendVerifyVersionCommand
{
    ...
    self.isSending = YES;
    [self sendStrDataToBle:@"{ 'command': 5, 'content': '{}' }"];
}
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    if(self.connectedPeripheral == peripheral && self.connectedPeripheral.state != CBPeripheralStateDisconnected)
    {
        [ViewUtils showToast:NSLocalizedString(@"already bound",nil)];
        return;
    }

//    else if (self.connectedPeripheral && identifier && [peripheral.identifier.UUIDString isEqualToString:self.connectedPeripheral.identifier.UUIDString]) {
//        NSLog(@"equlas");
//        return;
//    }
    ...
    {
        NSString *identifier = [[NSUserDefaults standardUserDefaults] objectForKey:kBLEBindingWatch];
        //如果以前连接过，但是现在断开了，重新搜索到后，直接发送连接命令
...
-(void) centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    //added by huxx
    [self.centralManager stopScan];
    [self.connectedPeripheral discoverServices:@[[CBUUID UUIDWithString:TRANSFER_SERVICE_UUID]]];
}
...
//        [peripheral discoverCharacteristics:nil  forService:s];
        ...
        [peripheral discoverCharacteristics:@[[CBUUID UUIDWithString:TRANSFER_CHARACTERISTIC_UUID]]  forService:s];
    }
}
...
- (void) peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    for (CBCharacteristic *characteristic in service.characteristics) {
        ...
        if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:TRANSFER_CHARACTERISTIC_UUID]]) {
            NSLog(@"bingo..");
            NSLog(@"the characteristic UUID is %@",characteristic.UUID);
